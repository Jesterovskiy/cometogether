...
class API::V1::ItemsController < ApplicationController
  ...
  def index
    ...
    render json: ResponsePresenter.new(items), status: 200
  end
  ...
  def show
    ...
    if item
      authorize item
      render json: ResponsePresenter.new(item), status: 200
    else
  ...
  def create
    ...
    if item.valid?
      ...
      render json: ResponsePresenter.new(error: item.errors, status: 400), status: 400
    end
  end
